...
const items = computed(() => prop.categories);
...
function confirmDelete() {
    axios.delete('http://localhost:8000/api/v1/categories/' + item.value);
...
const formUpdateSubmit = () => {
    axios.put('http://localhost:8000/api/v1/categories/' + form.id, {
        name: form.name,
        slug: form.slug,
    })
        .then((response) => {
...
<template>
    <CardBoxModal v-model="isModalUpdateActive" title="Update Category" has-cancel>
            ...
            <tr v-for="category in itemsPaginated" :key="category.id">
                ...
                <td data-label="Created_at" class="lg:w-1 whitespace-nowrap">
                    <small class="text-gray-500 dark:text-slate-400" :title="category.created_at">{{
                        moment(category.created_at).format("MMM Do YY") }}</small>
                </td>
                <td class="before:hidden lg:w-1 whitespace-nowrap">
                    <BaseButtons type="justify-start lg:justify-end" no-wrap>
                        <BaseButton color="info" :icon="mdiPencil" small @click="editDetails(category)" />
                        <BaseButton color="danger" :icon="mdiTrashCan" small @click="deleteItem(category.id)" />
                    </BaseButtons>
                </td>
            </tr>
        </tbody>

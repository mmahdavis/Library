...
const items = computed(() => prop.tags);
...
function confirmDelete() {
    axios.delete('http://localhost:8000/api/v1/tags/' + item.value);
...
const formUpdateSubmit = () => {
    axios.put('http://localhost:8000/api/v1/tags/' + form.id, {
        name: form.name,
        slug: form.slug,
    })
        .then((response) => {
...
<template>
    <CardBoxModal v-model="isModalUpdateActive" title="Update Tag" has-cancel>
    ...
    <table>
        ...
        <tbody>
            <tr v-for="tag in itemsPaginated" :key="tag.id">
                <TableCheckboxCell v-if="checkable" @checked="checked($event, tag)" />
                <td class="border-b-0 lg:w-6 before:hidden">
                    <UserAvatar :username="tag.name" class="w-24 h-24 mx-auto lg:w-6 lg:h-6" />
                </td>
                <td data-label="Name">
                    {{ tag.name }}
                </td>
                ...
                <td data-label="Created_at" class="lg:w-1 whitespace-nowrap">
                    <small class="text-gray-500 dark:text-slate-400" :title="tag.created_at">{{
                        moment(tag.created_at).format("MMM Do YY") }}</small>
                </td>
                <td class="before:hidden lg:w-1 whitespace-nowrap">
                    <BaseButtons type="justify-start lg:justify-end" no-wrap>
                        <BaseButton color="info" :icon="mdiPencil" small @click="editDetails(tag)" />
                        <BaseButton color="danger" :icon="mdiTrashCan" small @click="deleteItem(tag.id)" />
                    </BaseButtons>
                </td>
            </tr>
        </tbody>

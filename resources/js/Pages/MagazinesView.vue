...
async function getMagazines() {
    await axios.get('http://localhost:8000/api/v1/magazines').then((r) => { magazines.value = r.data; });
}
getMagazines()
...
const fromInsertSubmit = () => {
    axios.post('http://localhost:8000/api/v1/magazines', {
        name: form.name,
        slug: form.slug,
        image: form.image,
        price: form.price
...
<template>
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            <SectionTitleLineWithButton :icon="mdiBookMultipleOutline" title="Magazines" main>
            ...
            <CardBox v-if=magazines class="mb-6" has-table>
                <Magazines checkable :magazines=magazines.data @update-magazines="getMagazines()" />
            </CardBox>
            <CardBox v-else>

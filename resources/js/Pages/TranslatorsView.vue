...
import Translators from "@/components/Tables/Translators.vue";
...
async function getTranslators() {
    await axios.get('http://localhost:8000/api/v1/translators').then((r) => { translators.value = r.data; });
}
getTranslators()
...
const fromInsertSubmit = () => {
    axios.post('http://localhost:8000/api/v1/translators', {
        name: form.name,
        slug: form.slug,
        image: form.image,
    })
...
<template>
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <CardBoxModal v-model="isModalInsertActive" title="Add New Translator" has-cancel>
            ...
            <SectionTitleLineWithButton :icon="mdiBookMultipleOutline" title="Translators" main>
            ...
            <CardBox v-if=translators class="mb-6" has-table>
                <Translators checkable :translators=translators.data @update-translators="getTranslators()" />
            </CardBox>
            <CardBox v-else>

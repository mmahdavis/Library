...
import Writers from "@/components/Tables/Writers.vue";
...
async function getWriters() {
    await axios.get('http://localhost:8000/api/v1/writers').then((r) => { writers.value = r.data; });
}
getWriters()
...
const fromInsertSubmit = () => {
    axios.post('http://localhost:8000/api/v1/writers', {
        name: form.name,
        slug: form.slug,
        image: form.image,
    })
...
<template>
    <LayoutAuthenticated>
        <Head title="Writers" />
        <SectionMain>
            <CardBoxModal v-model="isModalInsertActive" title="Add New Writer" has-cancel>
            ...
            <SectionTitleLineWithButton :icon="mdiBookMultipleOutline" title="Writers" main>
            ...
            <CardBox v-if=writers class="mb-6" has-table>
                <Writers checkable :writers=writers.data @update-writers="getWriters()" />
            </CardBox>
            <CardBox v-else>
